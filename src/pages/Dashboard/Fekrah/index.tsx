...
export default function FekrahPage() {
  ...
  const [chat, setChat] = useState<MessageProps[]>([
    ...
    { id: 3, isRes: false, text: "Write about war" },
    {
      id: 4,
      isRes: true,
      text: `Certainly! War is a state of armed conflict between different nations or groups. It is a complex and multifaceted event that can have wide-ranging impacts on societies, economies, and political systems.

        Wars can be fought for a variety of reasons, Certainly! War is a state of armed conflict between different nations or groups. It is a complex and multifaceted event l disputes, ideological differences, or the desire for power and resources. The causes of war are often rooted in long-standing conflicts or grievances between nations or groups.
        `,
    },]);
  const [isLoading, setIsLoading] = useState(false);
  const messageContainerRef = useRef<HTMLDivElement>(null);
  ...
  return (
    <div className=" flex flex-col justify-between h-screen max-w-full flex-1">
      ...
      <div className=" bottom-8 w-full pb-12 px-10">
        {textfield}
      </div>
    </div>
  );
